...
namespace Shawn.ProjectFramework
{
    ...
    public class DialogueManager : IBaseManager
    {
        ...
        public void ShowDialogue(int pos)
        {
            ...
            if (temp == null)
            {
                PanelManager.Instance.ShowPanel<DialoguePanel>("DialoguePanel");
            }
        }
        ...
        private void Step()
        {
            //由于异步加载，变量赋值需放在显示面板后
            if (temp == null)
                temp = PanelManager.Instance.GetPanelByName("DialoguePanel");

            //如果文字正在滚动，将文字补全
            if (m_isRolling)
            {
                (temp as DialoguePanel).UpdateInfo(tempList[pos].Info);
                m_isRolling = false;
            ...
            {
                PanelManager.Instance.HidePanel("DialoguePanel");
                m_CanStep = false;
                return ;
            }
        ...
        {
            m_isRolling = true;
            (temp as DialoguePanel).UpdateInfo("");
            foreach (char c in dialogue_SO.Nodes[pos].Info)
            {
                (temp as DialoguePanel).AppendInfo(c);
